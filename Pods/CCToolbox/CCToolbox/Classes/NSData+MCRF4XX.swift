//
//  NSData+MCRF4XX.swift
//  Pods
//
//  Created by Kevin Tallevi on 6/5/17.
//
//

import Foundation

private var crcMCRF4XXTable: [UInt16] = [
...
func calculateMCRF4XX(data: NSData) -> UInt16 {
    var crc: UInt16 = 0xFFFF
    
    let bytePointer = data.bytes.assumingMemoryBound(to: UInt8.self)
    let bytes = UnsafeBufferPointer<UInt8>(start: bytePointer, count: data.length)
    
    for byte in bytes {
        let index = Int((UInt16(byte) ^ crc) & 0xFF)
        crc = crcMCRF4XXTable[index] ^ (crc >> 8 & 0xFF)
    }
...
extension NSData {
    public var crcMCRF4XX: NSData {
        let crc16: UInt16 = calculateMCRF4XX(data: self)
        var bytes:[UInt8] = []
        bytes.append(UInt8(crc16 & 0x00ff))
        bytes.append(UInt8(crc16 >> 8))
        
        let crc = NSData(bytes: bytes, length: bytes.count)
        return crc
    }
}
